...
def rebuild_models(autolabeled_positive_records_file_path, autolabeled_negative_records_file_path, already_processed_record_numbers_file_path, input_file_total_lines_count_file_path):
    ...
    if config.upload_output_to_remote_server == True:
        ...
        if os.path.exists(already_processed_record_numbers_file_path):
            files_to_upload.append(already_processed_record_numbers_file_path)

        if os.path.exists(input_file_total_lines_count_file_path):
            files_to_upload.append(input_file_total_lines_count_file_path)

        sharedlib.upload_files_to_labeling_auto_labeled_dir(files_to_upload)
